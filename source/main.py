    ...
    if right == 1 and not lost:
        ...
        if aiButton.collidepoint(mouse) and not lost:
            ...
            if (i is None) or (j is None): # means no safe move
                ...
                move = Cell(i,j)
                if move: #it's possibile to make random movement
        ...
        # User-made move
            ...
            for i in range(HEIGHT):
                for j in range(WIDTH):
                    if (cells[i][j].collidepoint(mouse) and Cell(i, j) not in flags and Cell(i, j) not in revealed):
                        move = Cell(i, j)
    ...
    if move:
        if game.is_mine(move):
            ...
            '''if nearby is not None:
                ...
                ai.add_proposition(move.row,move.col,nearby)
            else:
